...
```{r}
# 1: use read.csv() to read our dataset from /data_input/retail.csv
# name the dataframe `retail`
retail <- read.csv("data_input/retail.csv")

# 2: use as.Date(df$x, format="%x/%y/%z") to convert the date columns
# remember to reassign them (reminder: %d-day; %m-month; %y-year(2))
retail$Order.Date <- as.Date(retail$Order.Date, format="%m/%d/%y")
retail$Ship.Date <- as.Date(retail$Ship.Date, format="%m/%d/%y")

# create two new features, Month and Day of Week respectively
retail$Order.Month <- months(retail$Order.Date)
retail$Order.DOW <- weekdays(retail$Order.Date)

# 3: how many columns are there? Use any code appropriate for the task
# str(retail)

# 4. How many transactions do we have in the Furniture category?
table(retail$Category)
```


```{r}
students <- c("samuel", "andy", "samuel", "samuel")
unique(students)
length(unique(students))
```
**Dive Deeper Day 2**:

1. How many unique items are there in our transaction? Hint: try `unique(x$variable)` and combine that with `length()`
Answer: 1862

```{r}
length(unique(retail$Product.Name))
length(unique(retail$Product.ID))
```

2. How many transactions where we incur a loss of more than $50 (rugi)
Answer: 525

```{r}
nrow(subset(retail, Profit < -50))
```

3. What percentage of our transactions are shipped on First class?
Answer: 15.39

```{r}
fc <- subset(retail, Ship.Mode == "First Class")
nrow(fc)/nrow(retail) * 100
```

```{r}
prop.table(table(retail$Ship.Mode))['First Class'] * 100

# optionally, round to two digits
round(prop.table(table(retail$Ship.Mode))['First Class'] * 100, 2)
```

4. What is the maximum profit we have ever made from the `Technology` category? Hint: use `max()` after the right subsetting
Answer: 8399

```{r}
tech <- subset(retail, Category == 'Technology')
max(tech$Profit)
```

5. Create a dataframe called `sameday` containing only transactions shipped on same day. What percentage of them are to `Home Office`?
Answer: 20.62

```{r}
# this is a wrong approach
prop.table(table(retail$Segment, retail$Ship.Mode)) * 100
```

```{r}
#Cara 1
sameday <- retail[retail$Ship.Mode == "Same Day", ]
prop.table(table(sameday$Segment)) * 100
```

```{r}
#Cara 2
sameday <- retail[retail$Ship.Mode == "Same Day", ]
ho_samedy <- sameday[sameday$Segment == "Home Office", ]

nrow(ho_samedy) / nrow(sameday) * 100
```


Two ways to subset
1. `df[row, column]`
- `loans[loans$default == TRUE, 4:9]`
- `loans[1:10, 5]`
- `loans[loans$purpose == 'business', c("applicant_name", "npwp")]`

2. `subset(loans, purpose == 'business')`

```{r}
cond1 <- retail$Profit < -50
retail[cond1, c("Customer.ID", "Segment")]
```

What is the breakdown of Sales that occur from selling 'Furniture' across the different day of week? 
```{r}
table(retail$Category, retail$Segment)
```

Crosstab from `Sales` based on `Sub.Category` and `Ship.Mode`

```{r}
xtabs(Sales ~ Sub.Category + Ship.Mode, retail)
```
Heatmap from the crosstab above

```{r}
heatmap(xtabs(Sales ~ Sub.Category + Ship.Mode, retail), Colv = NA, Rowv = NA, cexCol = 0.6,scale = "column")
```
Breakdown our profit by shipping mode and sub-category. Make subcategory appear on the row.

```{r}
xtabs(Profit ~ Sub.Category + Ship.Mode, retail)
```
Crosstab from `Sales` based on `Order.DOW` and `Category`
```{r}
# cross-tabulation (`x`: cross)
xtabs(Sales ~ Order.DOW + Category, retail)
```

```{r}
retail$Order.DOW <- weekdays(retail$Order.Date)
plot(xtabs(Sales ~ Order.DOW + Category, retail))
```


```{r}
table(retail$Category, retail$Order.DOW)
```

Create a df called 'startups', where the data contain transactions not in the Corporate segment

1. check what `Segments` we have on `retail` data

```{r}
table(retail$Segment)
```

2. Check the `unique` item in `Category`

```{r}
unique(retail$Category)
```

3. Make a dataset called `startups`
```{r}
# nrow(subset(retail, Category == 'Furniture'))
# create a df called 'startups' that contain transactions not in the Corporate segment
startups <- subset(retail, Segment != "Corporate")
nrow(startups)
```

```{r}
corporate <- subset(retail, Segment == 'Corporate')
nrow(retail) - nrow(corporate)
```

```{r}
startups <- subset(retail, Segment == "Home Office" | Segment == "Consumer")
nrow(startups)
```

```{r}
# create a df called highvalue, find transactions that have a profit of 5000 or more. How many transactions are in highvalue?
highvalue <- subset(retail, Profit >= 5000)
nrow(highvalue)
```

```{r}
# subset the first 10 rows, and the 2nd, 5th, 7th, 9th columns
# retail[1:10, c(2,5,7,9)]
retail[3:6, c("Order.ID", "Order.Date")]
```
...
```{r}
subset(retail, Order.Date < as.Date("2015-01-01"))
```
...
```{r}
retail[order(retail$Order.Date, decreasing = TRUE), ]
```



```{r}
subset(retail, Profit < 0 & (Segment == "Home Office" | Ship.Mode == "Second Class"))
```
