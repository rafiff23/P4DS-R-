...
CTRL / CMD + ALT/ options + i = insert
CTRL / CMD + SHIFT + enter = run the chunk
...
Create a simple vector of 4 elements, containing strings of common first names. Name the vector `clients`, then run `class(clients)`, what do you get as the output?

```{r}
clients <- c("andi", "bagas", "charlie", "deni")
class(clients)
```
...
```{r}
clients <- c("andi", "bagas", "charlie", "deni", 1234)
print(clients)
```

```{r}
attendance <- c(TRUE, TRUE, 0, TRUE, 1)
print(attendance)
class(attendance)
```
...
```{r}
# Your code here:
customers <- c("andi", "bagas", "charlie", "dede")
age <- c(31, 30, 36, 24)
```
...
```{r}
# Your code here
suppliers <- c("map", "starbucks", "kenangan")
```
...
```{r}
# 3: clean data. Make sure dates are recognized as dates in a valid format
tail(retail$Order.Date)
```

It turns out the format is `%m/%d/%y`, go ahead and use `as.Date()` to convert the two date columns into the right class (date type)


```{r}
table("Kategori"=retail$Category, retail$Ship.Mode)
```
```{r}
dim(retail)
```

1. How many transactions are shipped First Class to the Corporate segment?
Bonus: What is that in terms of percentage?
```{r}
...
```{r}
table1 <- table(retail$Ship.Mode)
prop.table(table1)
```



2. How many SKUs (unique products) are there in our dataset?
```{r}
...
#atau
startups2 <- subset(retail, Segment == 'Corporate' | Segment == 'Consumer')
startups
startups2
```
...
#3
nrow(subset(retail, Ship.Mode == "First Class"))
#atau
...
#5
sort(subset(retail, Category == "Technology"), decreasing = TRUE)
sameday <- table(retail$Ship.Mode)
prop.table(sameday)
```

```{r}
subset(retail, Profit <= -50)
```
...
```{r}
Order.DOW = weekdays(retail$Order.Date)
```
